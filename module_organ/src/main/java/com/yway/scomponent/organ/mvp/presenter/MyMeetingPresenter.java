...
public class MyMeetingPresenter extends BasePresenter<MyMeetingContract.Model, MyMeetingContract.View> {
    ...
    public void queryMyMeetingPageList(Map<String, Object> params, boolean pullToRefresh){
        if (pullToRefresh){
            pageNo = 1;
        }else{
        ...
        mModel.queryMyMeetingPageList(params)
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<ConferenceBean>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<ConferenceBean> datas) {
                        //文档列表是否为空

...
public class WaterMarkView extends View {
    ...
    private String str = "智慧机关";//水印文字
    private float hSpace = 150f;//水平间距
    private float vSpace = 100f;//垂直间距
    private int mScaleSize = 2;//缩放大小 固定：2
    private int mDegrees = -30;//旋转角度
    private int bgColor = Color.TRANSPARENT;//背景颜色
    private int textColor = Color.parseColor("#30A4A4A4");//字体颜色
    private float textSize = 40f;//字体大小
    ...
    public void init(Context context, AttributeSet attrs) {
        if (null != attrs) {
            ...
            hSpace = ta.getDimension(R.styleable.WaterMarkView_hSpace, hSpace);
            vSpace = ta.getDimension(R.styleable.WaterMarkView_vSpace, vSpace);
            mDegrees = ta.getInteger(R.styleable.WaterMarkView_degrees, mDegrees);
            bgColor = ta.getColor(R.styleable.WaterMarkView_bgColor, bgColor);
            textColor = ta.getColor(R.styleable.WaterMarkView_textColor, textColor);
            textSize = ta.getDimension(R.styleable.WaterMarkView_textSize, textSize);
            ta.recycle();
        }
    }
    ...
    private void drawWaterMark(Canvas canvas) {
        str = CacheUtils.queryUserNameAddPhone();
        ...
        for (int i = 0; i < hSize; i++) {
            ...
            for (int j = 0; j < vSize; j++) {
                ...
                if (j%2==0){
                    ...
                    canvas.drawText("榆阳智云", xDraw, yDraw, paint);
                }
            }
        }
        canvas.restore();
